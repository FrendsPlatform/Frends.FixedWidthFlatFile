...
public static class FixedWidthFlatFile
{
    ...
    /// <returns>Object { bool Success, Error Error, List&lt;Dictionary&lt;string Key, object Value&gt;&gt; Data }</returns>
    public static Result Parse([PropertyTab] Input input, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return new Result(true, outputData);
        }
        catch (Exception ex)
        {
    ...
    internal static Dictionary<string, object?> ParseDataRow(string row, ColumnSpecification[] columnSpecifications)
    {
    ...
    internal static void AddKeyValuePair(this Dictionary<string, object?> dictionary, string key, object? value)
    {
        var originalKey = key;
        int renameIndex = 1;
